...
function LoadDataTablesScripts(callback){
	function LoadDatatables(){
		...
		$.getScript('plugins/DataTables-1.10.6/media/js/jquery.dataTables.js', function(){
			$.getScript('plugins/datatables/ZeroClipboard.js', function(){
				$.getScript('plugins/DataTables-1.10.6/extensions/TableTools/js/dataTables.tableTools.js', function(){
					$.getScript('plugins/datatables/dataTables.bootstrap.js', callback);
				});
			});
		});
		// $.getScript('plugins/DataTables-1.10.6/extensions/dataTables.rowsGroup.js', function(){ });
		// $.getScript('plugins/numeral/jquery.number.js', function(){ });
...
//
function LoadAjaxContent(url,modul){
	//alert(modul);
	$('.preloader').show();
	$.ajax({
...
$(document).ready(function () {
	...
	var ajax_u = location.hash;
	var urlx= ajax_u.split('#');
	...
	LoadAjaxContent(ajax_url,modul);
	$('.main-menu').on('click', 'a', function (e) {
		...
		if ($(this).hasClass('ajax-link')) {
			...
			var url2 = url.split('#');
			window.location.hash = url2[0];
			LoadAjaxContent(url2[0],url2[1]);
		}
		if ($(this).attr('href') == '#') {
			e.preventDefault();
		}
